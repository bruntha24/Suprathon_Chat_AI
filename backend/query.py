...
# Greeting words
GREETING_WORDS = [
    "hi", "hello", "hey", "hii", "helo", "hey there", "hi there",
    "good day", "yo", "yoo", "heyyo","hiii","boo","good evening","good afternoon","good morning","good night","gud morning","gud afternoon","gud evening","gud night","good morining"
]
def get_answer(user_query: str):
    query_clean = clean_text(user_query)   # clean the input
    query_words = query_clean.split()      # split words

    # -----------------------------
    # 1️⃣ Emoji-only check
    # -----------------------------
    # Remove spaces to check if only emojis/symbols
    query_no_space = re.sub(r"\s+", "", user_query)
    # Remove letters/numbers/punctuation
    query_letters = re.sub(r"[a-zA-Z0-9.,!?;:]", "", query_no_space)

    if len(query_letters) > 0 and len(query_letters) == len(query_no_space):
        # Input is mostly emojis
        return "👋 Hmm ! For more information, please visit the official website: https://suprathon.suprazotech.in"

    # -----------------------------
    # 2️⃣ Greeting Detection
    # -----------------------------
    for g in GREETING_WORDS:
        if query_clean.startswith(g):
            return f"👋 {g.capitalize()}... How can I help you today?"

    # -----------------------------
    # 3️⃣ Multi-part question handling
    # -----------------------------
